...
add_action( 'genesis_meta', 'll_archive_team_template' );

function ll_archive_team_template() {

	remove_action( 'genesis_loop', 'genesis_do_loop' );
	add_action( 'genesis_after_header', 'll_team_entry_background' );
	add_action( 'genesis_before_loop', 'll_add_archive_team_content' );
...
function ll_team_entry_background() {
	...
	echo '<div class="entry-background"><img src="' . $teamimage['sizes']['feature-large'] . '" alt="' . $teamimage['alt'] . '"></div>';
	echo '</div>';
}

function ll_add_archive_team_content() {

	$template = get_option( 'template' );

	while ( have_posts() ) : the_post();

		wpfm_get_template( 'teams/content-team.php', array( 'team' => new WPFM_team( get_the_ID() ) ) );
...
function change_to_senior_teams_sidebar() {
	...
	if( is_active_sidebar( 'senior-teams-sidebar' ) ) {
		...
		add_action( 'get_header', 'll_senior_teams_sidebar' );
	}
}
